...
class Quaternions:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Quaternions):
            ...
            qs[...,0] = r3 * q0 + r0 * q3 - r1 * q2 + r2 * q1
            qs[...,1] = r3 * q1 + r0 * q2 + r1 * q3 - r2 * q0
            qs[...,2] = r3 * q2 - r0 * q1 + r1 * q0 + r2 * q3
            qs[...,3] = r3 * q3 - r0 * q0 - r1 * q1 - r2 * q2
    ...
    def __neg__(self):
        ...
        """the same rotation in the opposite orientation"""
        return Quaternions(self.qs * np.array([[-1, -1, -1, -1]]))

    def __abs__(self):
        """ Unify Quaternions To Single Pole """
        qabs = self.normalized().copy()
        top = np.sum(( qabs.qs) * np.array([0,0,0,1]), axis=-1)
        bot = np.sum((-qabs.qs) * np.array([0,0,0,1]), axis=-1)
    ...
    def rot(self, other):
        if isinstance(other, np.ndarray) and other.shape[-1] == 3:
            other = Quaternions(np.concatenate([other, np.zeros(other.shape[:-1] + (1,))], axis=-1))
        return (self * other * self.inv()).imaginaries
    ...
    def between(cls, v0s, v1s):
        ...
        return Quaternions(np.concatenate([a, w[...,np.newaxis]], axis=-1)).normalized()
    ...
    def from_angle_axis(cls, angles, axis):
        ...
        return Quaternions(np.concatenate([axis * sines, cosines], axis=-1))
